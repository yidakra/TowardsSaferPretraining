...
\hline
\end{tabular}
\caption{Summary of Reproducibility Results. $^*$Evaluated on TTP-Eval instead of the unreleased internal test split. $^{\dagger}$April measurement (TTP-Eval $n=393$ via OpenRouter; OpenAI Mod $n=1{,}680$ via OpenRouter, $P=0.80$, $R=0.42$, $F_1=0.55$). $^{\ddagger}$May re-run on byte-identical code (Section~\ref{sec:model-drift}); both gaps close, attributable to silent \texttt{gpt-4o} snapshot drift. $^{**}$May OpenAI Mod re-run via OpenRouter: $P=0.75$, $R=0.93$, $F_1=0.83$ on 1{,}679/1{,}680 evaluated samples.}
...
\hline
Model & Eval/Total & Parse errs & $F_1$ audit [95\% CI] & Paper $F_1$ & \textsc{Intent} rate & Pred/gold toxic \\
\hline
...
\hline
\end{tabular}
\caption{Cross-model audit summary on TTP-Eval ($n=393$) with 95\% bootstrap CIs ($n=10{,}000$ resamples). \textsc{Intent} rate is the fraction of 1{,}955--1{,}965 filled dimension-slots (5 harm categories $\times$ evaluated samples) that the model labels as \textsc{Intent-*}. Pred/gold toxic columns are the fractions of \emph{samples} flagged toxic by aggregating the five categories. All four audited F1s contain the original paper's reported value within their 95\% CI, and the rank ordering by Intent rate matches the rank ordering by point-estimate F1.}
