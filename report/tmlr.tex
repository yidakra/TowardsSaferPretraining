...
Our extensions strengthen this picture in two ways. The cross-model study shows that TTP's apparent GPT-family lock-in is largely a parser-format artifact: open-weight models (DeepSeek-R1, Gemini Flash) can match GPT-4o once output formatting is hardened, contradicting the impression Mendu et al.'s Table~4 leaves that R1 and Gemma fail at the task. The multilingual study shows the opposite: HarmFormer is genuinely English-bound (0.21--0.40 F1 across six non-English languages), which means responsible-AI deployments outside English need either retraining, prompt adaptation, or human-verified multilingual analogs of TTP-Eval before adopting any of the released tools.
