...
\textbf{Cross-model parser caveat.} The Gemma 2 27B failure is partly explained by output-format mismatch with the TTP parser; we did not engineer a Gemma-specific parser to recover the lost recall. The 0.35 F1 we report is therefore a lower bound, and the gap to GPT-4o is in part an artefact of the released parser rather than a property of the model.
