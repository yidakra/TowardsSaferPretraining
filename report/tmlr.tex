...
\textbf{TTP API endpoint.} The paper queries GPT-4o through the OpenAI API directly. We query the same model through OpenRouter because that is the API key our institution makes available. Endpoint differences (routing, undocumented decoding parameters, regional availability) are a plausible source of the recall gap reported in Section~\ref{sec:results}.
...
Table~\ref{tab:reproducibility-summary} summarizes the per-claim outcome. Only Claim 4 (HAVOC leakage) reproduces within tolerance; Claim 2 (HarmFormer) is partially validated against a publicly-available proxy benchmark; Claims 1 and 3 (TTP) diverge by more than 20 F1 points, with the divergence concentrated in recall rather than precision. The pattern --- public artifacts that wrap a closed API show systematic recall loss when re-run by an independent party --- is the load-bearing finding of this study, and motivates the cross-model and multilingual experiments in the next two sections.
...
\textbf{Accountability.} The reproduction shows that headline numbers reported under the same name can differ by 20+ F1 points when re-run by an independent party against the same publicly released artifacts. This is not a moral judgement of the original authors; it is a description of what a current reader can verify. The practical remedy is to publish pre-computed generations and judge artifacts (as the HAVOC \texttt{havoc\_modeleval.tsv} release does) wherever the underlying models are not themselves redistributable.
...
\textbf{TTP recall gap is unresolved.} We attribute the 21-point F1 gap on TTP-Eval to endpoint variance and undocumented decoding parameters (Section~\ref{sec:deviations}), but we cannot prove this without access to the authors' direct OpenAI logs. Prompt-sensitivity experiments and head-to-head OpenAI-vs-OpenRouter runs would tighten the diagnosis.
