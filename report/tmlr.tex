...
Our specific contributions are: \textbf{(i)} a per-claim reproducibility table (Table~\ref{tab:reproducibility-summary}) showing that one of four claims reproduces within tolerance, two diverge by more than 20 F1 points in the original measurement (one of which shrinks to ${\sim}5$ F1 points on a re-run two weeks later, surfacing silent \texttt{gpt-4o} snapshot drift), and one is partially validated against a publicly available proxy benchmark; \textbf{(ii)} a public-artifact HarmFormer baseline on TTP-Eval (0.78 F1), which the original paper does not publish and which is needed for any future cross-model comparison; \textbf{(iii)} a cross-model TTP generalization study on five additional LLMs that disentangles prompt-format failures from semantic ones; \textbf{(iv)} a multilingual robustness study covering six languages and four scripts, addressing the multilingual gap that the original paper explicitly defers to future work; and \textbf{(v)} fully tracked experiments (Weights \& Biases run links, CodeCarbon energy traces, Slurm job scripts) released alongside the code so that every reported number can be regenerated by an independent reader.
...
This is a 25-point F1 deficit relative to the published 0.72 P, 0.91 R, 0.80 F1, again driven by recall rather than precision. Coupled with the matching pattern on TTP-Eval (Claim 1), it suggests the gap is systematic rather than benchmark-specific: TTP-as-deployed-by-us is recall-conservative compared to TTP-as-evaluated-by-the-authors. Section~\ref{sec:model-drift} rules out endpoint variance as the explanation on TTP-Eval, where a May re-run on \texttt{gpt-4o} narrows the gap to within five F1 points across both endpoints. The OpenAI Moderation result reported here is from the same April measurement window and queries the same model, so the same silent \texttt{gpt-4o} snapshot drift very plausibly accounts for most of this gap as well; we did not re-run OpenAI Moderation. Other contributing factors may include undocumented decoding parameters or post-hoc filtering on the authors' side that the released prompt does not encode.
...
Table~\ref{tab:reproducibility-summary} summarizes the per-claim outcome. Only Claim 4 (HAVOC leakage) reproduces within tolerance; Claim 2 (HarmFormer) is partially validated against a publicly-available proxy benchmark; Claims 1 and 3 (TTP) diverge by more than 20 F1 points in the original measurement, with the divergence concentrated in recall rather than precision. Section~\ref{sec:model-drift} shows that Claim 1's gap shrinks to roughly five F1 points two weeks later under both OpenAI direct and OpenRouter endpoints, attributable to silent \texttt{gpt-4o} snapshot drift. The pattern --- public artifacts that wrap a closed API show systematic recall loss when re-run by an independent party, and the magnitude of that loss varies meaningfully with silent model updates --- is the load-bearing finding of this study, and motivates the cross-model and multilingual experiments in the next two sections.
...
\hline
1: TTP on TTP-Eval & F1 & 0.83 & 0.62$^{\dagger}$ & Not reproduced$^{\dagger}$ \\
...
\multicolumn{5}{l}{\small $^{\dagger}$April measurement; a May re-run on byte-identical code yields $F_1 \in [0.78, 0.79]$ (Section~\ref{sec:model-drift}).} \\
\end{tabular}
...
\begin{itemize}
    ...
    \item Cross-model TTP performance is sensitive both to output formatting (e.g., GPT-OSS 20B's 8/393 parse failures) and to model-level toxicity-labelling conservatism (Gemma 2 27B), with these two effects sometimes mistaken for each other.
...
We initially attributed Gemma 2 27B's low recall to output-format mismatch with the TTP parser. To verify, we instrumented the local TTP client to log raw model outputs and re-ran the first 20 TTP-Eval samples. Every output was a well-formed \texttt{<Label>\{H: \dots, IH: \dots, SE: \dots, IL: \dots, SI: \dots\}</Label>} block whose values parsed cleanly to the canonical \textsc{none}/\textsc{topical}/\textsc{intent} dimensions---zero parse errors, zero silent fallbacks. The recall gap is therefore not a parser artefact: Gemma 2 27B simply emits \textsc{Topical-*} for content GPT-4o flags as \textsc{Intent-*}, predicting any toxic dimension on only 2 of the 20 audit samples versus a ground-truth rate of 12/20. The 0.35 F1 reflects genuine model-level conservatism in flagging toxicity, consistent with Mendu et al.'s observation that the prompt is GPT-tuned but with a sharper diagnosis: the bottleneck is the model's labelling policy, not output formatting. The audit JSON (\texttt{audit\_gemma\_n20.json}) is released alongside the code.
...
\begin{enumerate}
\setlength{\itemsep}{0pt}
\item \emph{Format-driven cliffs}: GPT-OSS 20B reaches 0.96 precision but only 0.24 recall, and 8 of 393 samples produce no parseable output at all. The model is conservative in flagging harm and frequently generates verbose reasoning without the required \texttt{<Label>} block.
\item \emph{Model-level conservatism}: Gemma 2 27B's 0.35 F1 is \emph{not} a parser-format artefact (the raw-output audit above shows clean parsing on every sample) but a property of the model: it almost always emits \textsc{Topical-*} rather than \textsc{Intent-*}. Switching to Gemma 3 27B, with the same prompt and parser, lifts F1 to 0.57, suggesting model-generation improvements reduce but do not eliminate the conservatism gap to GPT-class models.
\end{enumerate}

The strongest result is that DeepSeek-R1-Distill-Qwen-32B (0.63 F1) matches GPT-4o (0.62 F1) and Gemini 2.0 Flash (0.80 F1) outperforms it, which directly contradicts the impression Table 4 leaves that TTP is GPT-family-locked. The pattern across the six models suggests two distinct sources of cross-model variance: actual parser brittleness for models that produce malformed output (GPT-OSS), and intrinsic labelling conservatism for models that produce well-formed output but rarely select the toxic dimension (Gemma 2 27B). The first is addressable by parser engineering or stricter system prompts; the second is not, and reflects a real semantic gap between the model's notion of ``toxic'' and the TTP taxonomy. We view this as actionable: practitioners without GPT-4o access can still deploy TTP on the parser-friendly open models (DeepSeek-R1, Gemini Flash, Gemma 3 27B), but should not assume any open-weight model will reach GPT-class recall.
...
The easiest components to reproduce were those with released artifacts and minimal inference requirements: HAVOC leakage computed from provided generations, and HarmFormer inference with the published weights. By contrast, API-dependent evaluations (TTP on TTP-Eval and OpenAI Moderation) were harder, owing to silent model-snapshot drift in \texttt{gpt-4o} (the same prompt, parser, and code returned $F_1=0.62$ in April and $F_1\in[0.78, 0.79]$ in May; Section~\ref{sec:model-drift}), undocumented decoding parameters, and occasional API failures that disproportionately depressed recall. Local TTP generalisation was also challenging, though for narrower reasons than we initially thought: GPT-OSS 20B genuinely required output-format hardening (8/393 samples emitted no parseable \texttt{<Label>} block), while Gemma 2 27B parsed cleanly but rarely flagged the toxic dimension---a model-policy effect, not a parser one (Section~\ref{sec:cross-model}). Memory constraints on the cluster also forced dtype and context-length tuning. These factors explain why artifact-based benchmarks replicated cleanly while prompt- and API-driven results were more fragile.
...
\textbf{TTP recall gap shifts with model snapshot.} The 21-point F1 gap on TTP-Eval observed in our April run shrinks to roughly five F1 points two weeks later under both endpoints (Section~\ref{sec:model-drift}). We cannot resolve whether the residual five-point gap is due to a still-unknown \texttt{gpt-4o}-snapshot difference between our May re-run and Mendu et al.'s original measurement, or to undocumented decoding parameters, since the authors' run logs are not available. What our A/B does establish is that endpoint variance is not the explanation.
...
\textbf{Cross-model audit caveat.} An earlier draft attributed Gemma 2 27B's 0.35 F1 to output-format mismatch with the TTP parser. A raw-output audit (n=20) refuted this: every output was a well-formed \texttt{<Label>} block that parsed cleanly, so the recall gap reflects genuine model-level conservatism rather than a parser artefact. The 0.35 F1 we report is therefore not a parser-induced lower bound. The audit was n=20 of 393 samples; a fuller audit with raw-output capture across the full benchmark would tighten the claim, but the failure mode is uniform across the audited subset.
...
This study reproduces \citet{mendu2025saferpretraining}'s framework as it appears from the public surface, and locates each of its four core claims on a verifiable--unverifiable spectrum. Claims that hinge on released, locally runnable artifacts (HAVOC leakage, HarmFormer inference) reproduce; claims that hinge on closed APIs or unreleased internal splits (TTP on TTP-Eval, OpenAI Moderation) diverge by 20+ F1 points in the original measurement, with the gap concentrated in recall and one of the two TTP gaps shrinking to ${\sim}5$ F1 points on a re-run that surfaces silent \texttt{gpt-4o} snapshot drift (Section~\ref{sec:model-drift}). Two additional experiments---cross-model TTP and multilingual TTP-Eval---show that the open-weight portability and language-coverage stories are richer than Mendu et al.'s Table~4 and Limitations section suggest: TTP is recoverable on at least two non-GPT backbones (DeepSeek-R1, Gemini Flash) and one parser-friendly Gemma generation (Gemma 3 27B), with the Gemma 2 27B outlier traced by raw-output audit to model-level conservatism rather than parser incompatibility, and benchmark mismatch rather than language is the dominant axis of variance for Llama Guard. We make every number in this paper independently re-runnable through Weights~\&~Biases, CodeCarbon, and Slurm-script artifacts, so a future reader can verify or refute any of these claims rather than take them on the same trust we did not extend to the original.
...
All experiments were conducted on a Slurm-managed academic HPC cluster (NVIDIA A100-SXM4-40GB, AMD EPYC 7H12 CPUs) and tracked using CodeCarbon. Appendix Table~\ref{tab:compute} summarises runtime and energy consumption. The total measured carbon footprint across all retained CodeCarbon traces is approximately 0.19~kg~CO$_2$, equivalent to driving roughly 0.8~km in an average passenger vehicle. API costs for TTP via OpenRouter totalled \$12.57 USD across 393 requests (${\sim}$1.57M tokens) for the April run, plus an additional ${\sim}$\$13 across the May OpenAI-direct and OpenRouter A/B re-runs.
...
TTP via OpenRouter (May A/B re-run)              & 1,150.6  & 0.0548 & 0.0147 \\
TTP via OpenAI direct (May A/B re-run)           & 4,537.6  & 0.2438 & 0.0652 \\
Cross-model TTP (Gemma 2 27B local, $n{=}20$ audit) & 67.4  & 0.0066 & 0.0018 \\
Multilingual eval (HarmFormer + Llama Guard, 6 langs) & 616.8 & 0.0343 & 0.0092 \\
